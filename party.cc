...
void Party::add_member(Actor *actor) {
	...
	if (actor->type() == "monster") monsters++;
	else if (actor->type() == "hero") heroes++;
}
...
float Party::weather_scale(string weather) {
	...
	if (weather == "Clear")
		...
		return 0.9; // rain slippery
	else if (weather == "Fog")
		return 0.85; // fog blinds vision
	else if (weather == "Windy")
		return 1.1; // windy more "smooth-moving"
	else
		return 1; // clear as a fallback
}
...
void Party::post_mortem(Actor *actor, bool gen_drop = true) {
	...
	if (!gen_drop) {
		if (actor->type() == "drop" || actor->type() == "merchant" ||
			actor->type() == "wall")
			delete actor;
		return;
	}
...
void Party::corpse_incinerator(bool forced = false) {
	if (dead_count < 2 && !forced)
		return;

	// CLL cleanup
	turn_order.reset_current_to_start();
	for (std::pair<Actor *, bool> actor_pair;
		 (actor_pair = turn_order.current(),
		 (actor_pair.first && actor_pair.second == false));) {
		Actor *actor = actor_pair.first;
		
		if (!actor)
			continue;
		else if (actor->is_dead()) {
...
void Party::inator() {
	...
	auto interactable = [&](const Actor *actor) {
		return (actor->type() == "monster") || (actor->type() == "hero") ||
			   (actor->type() == "merchant") || (actor->type() == "drop");
	};
	...
	if (cll_ptr)
		turn_order = std::move(*cll_ptr); // move, or point to the void
}

void Party::one_more_time() {
	// assumes bank r non-null and ALL LIVING.
	// HAVE TO PROCESS TO MATCH INVARIANT B4 EXIT
	if (status != ongoing && status != init)
		return;
	status = (cycles_left && status != hero_wins && status != monster_wins)
				 ? ongoing
				 : cycle_ends;
	if (status == cycle_ends)
		return;

	corpse_incinerator();

	auto actor_pair = turn_order.current();
	if (actor_pair.second)
		cycles_left--;
	Actor *actor = actor_pair.first;
	...
	auto fightable = [&actor](Actor *opponent) {
		...
		bool monster_hits_hero =
			(actor->type() == "monster") && (opponent->type() == "hero");
